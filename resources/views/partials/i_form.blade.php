...
@forelse($identifications as $question)
            ...
            <div class="card-body">
                ...
                <form class="forms-sample material-form">
                    <div class="form-group">
                        <input type="text" required="required" wire:model="answers.question_{{ $question->id }}" id="question_{{ $question->id }}" class="@if ($errors->has('answers.question_{{ $question->id }}')) text-danger @else text-primary @endif" autocomplete="off">
                        <label for="input" class="control-label">Enter answer here</label><i class="bar"></i>
                        @if ($errors->has('answers.' . 'question_'.$question->id))
